...
const Myplan = () => {
  ...
  const minutes = ()=>{
    if(click==="Today"){
      const newtime = todayplan.reduce((total,item)=> total + item.duration ,0)
      return newtime
    }
    else{
     const newtime = save.reduce((total,item)=> total + item.duration ,0)
      return newtime
    }
  }
  const calories = ()=>{
     if(click==="Today"){
      const newcalories = todayplan.reduce((total,item)=> total+item.caloriesBurned,0)
      return newcalories
    }
    else{
     const newcalories = save.reduce((total,item)=> total+item.caloriesBurned,0)
      return newcalories
    }
  }
  ...
  return (
    <div className='bg-[#0C0D10]'>
      <div className='container mx-auto my-10'>
        ...
        {
          click === "Today" ? <Todayplancard /> : <SaveCard />
